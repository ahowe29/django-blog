...
class SinglePostsView(View):
    ...
    def post(self, request, slug):
        ...
        if comment_form.is_valid():
            comment = comment_form.save(commit=False)  # commit false will not save it to the db just yet
            # we wait to save to db so we can add the post it's associated with to it first
            comment.post = post
            comment.save()
